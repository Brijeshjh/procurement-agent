...
def build_graph():
    ...
    # Add nodes
    workflow.add_node("requirement", process_requirement)
    workflow.add_node("vendor", find_vendors)
    workflow.add_node("rfq", generate_rfq)
    workflow.add_node("quotation", collect_quotations)
    workflow.add_node("comparison", compare_quotations)
    workflow.add_node("po", generate_po)
    
    # Define edges
    workflow.set_entry_point("requirement")
    workflow.add_edge("requirement", "vendor")
    workflow.add_edge("vendor", "rfq")
    workflow.add_edge("rfq", "quotation")
    workflow.add_edge("quotation", "comparison")
    workflow.add_edge("comparison", "po")
    workflow.add_edge("po", END)
